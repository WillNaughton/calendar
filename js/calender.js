$(document).ready(function() {
    ...
    $(".center-cal .cal").each(function(e) {
        if (e != 3) {
            $(this).hide();
        }
    });
    ...
    $(".next").click(function() {
        if ($(".center-cal .cal:visible").next().length != 0) {
            $('.center-cal .cal:visible').next().show().prev().hide();
        } else {
            $(".center-cal .cal:visible").hide();
            $(".center-cal .cal:first").show();
        }
        return false;
    });

    $(".prev").click(function() {
        if ($(".center-cal .cal:visible").prev().length != 0) {
            $('.center-cal .cal:visible').prev().show().next().hide();
        } else {
            $(".center-cal .cal:visible").hide();
            $(".center-cal .cal:last").show();
        }
        return false;
    });
